  ...
  try {
    ...
    if (typeof id !== 'number' || id <= 0) {
      res.status(400).json({
        success: false,
        error: 'ID inválido',
        details: 'Debes proporcionar un ID numérico del video.',
      });
      return;
    }
    ...
    if (typeof productId === 'number') {
      ...
      if (productError) {
        console.error('[update-product-video] Error fetching product:', productError);
        res.status(500).json({
          success: false,
    ...
    if (updateError) {
      console.error('[update-product-video] Error updating video:', updateError);
      res.status(500).json({
        success: false,
    ...
    if (!updatedVideo) {
      res.status(404).json({
        ...
        details: `No existe un video con ID ${id}`,
      });
      return;
    }
    ...
    console.error('[update-product-video] Unexpected error:', error);
    res.status(500).json({
      success: false,
