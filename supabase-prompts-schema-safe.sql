-- ============================================
-- SCHEMA PARA SISTEMA DE PROMPTS (VERSIÓN SEGURA)
-- ============================================
-- Ejecuta este script en SQL Editor de Supabase
-- Para la Fase 1: Configuración de Prompts
-- Esta versión evita operaciones DROP para que Supabase no muestre advertencias
...
-- ============================================
-- Crear triggers solo si no existen (usando DO block)
DO $$
BEGIN
  -- Trigger para system_prompts
  IF NOT EXISTS (
  ...
  -- Trigger para prompt_variables
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'update_prompt_variables_updated_at'
  ) THEN
...
BEGIN
  IF NOT EXISTS (
    SELECT 1 FROM pg_trigger 
    WHERE tgname = 'ensure_single_active_prompt_trigger'
  ) THEN
...
-- Crear políticas (si ya existen, se recrean con los mismos valores)
-- Esto es seguro porque solo afecta las políticas, no los datos
DO $$
BEGIN
