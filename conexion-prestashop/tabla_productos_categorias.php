...
function mapear_producto(array $producto, array &$cache_categorias)
{
    ...
    // Obtener todas las categorías del producto, excluyendo "Inicio" (ID 2)
    if (!empty($producto['associations']['categories'])) {
        foreach ($producto['associations']['categories'] as $cat) {
            $cat_id = is_array($cat) ? ($cat['id'] ?? null) : $cat;
            if ($cat_id && $cat_id != 2) { // Excluir categoría "Inicio" (ID 2)
                $nombre_cat = obtener_nombre_categoria((int) $cat_id, $cache_categorias);
                if ($nombre_cat && strtolower($nombre_cat) !== 'inicio') {
    ...
    if (empty($categorias) && !empty($producto['id_category_default']) && $producto['id_category_default'] != 2) {
        $nombre_cat = obtener_nombre_categoria((int) $producto['id_category_default'], $cache_categorias);
        if ($nombre_cat && strtolower($nombre_cat) !== 'inicio') {
            $categorias[] = $nombre_cat;
        }
    }
...
/**
 * Obtiene 100 productos de la API de PrestaShop.
 *
 * @return array
 */
